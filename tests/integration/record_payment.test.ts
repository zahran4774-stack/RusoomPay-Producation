// اختبارات record_payment — الدالة الأخطر في النظام
// نستدعي RPC عبر fx.asAccountant (جلسة JWT حقيقية) لأن الدالة تعتمد داخلياً على auth.uid()
...
describe('record_payment — الحسابات الأساسية', () => {
  it('يخصم المبلغ من الفاتورة بدقّة (paid يزيد بالضبط بقيمة الدفعة)', async () => {
    const { data, error } = await fx.asAccountant.rpc('record_payment', {
  ...
  it('يُنشئ قيداً محاسبياً متوازناً (مدين = دائن) لكل دفعة', async () => {
    await fx.asAccountant.rpc('record_payment', { p_fee_id: fx.feeId, p_amount: 25, p_method: 'bank' })
  ...
  it('يستخدم حساب النقد (1110) للدفع نقداً، والبنك (1120) لغير ذلك', async () => {
    await fx.asAccountant.rpc('record_payment', { p_fee_id: fx.feeId, p_amount: 10, p_method: 'cash' })
...
describe('record_payment — منع الأخطاء المالية', () => {
  it('يرفض دفعة بمبلغ صفر أو سالب', async () => {
    const { error } = await fx.asAccountant.rpc('record_payment', { p_fee_id: fx.feeId, p_amount: 0, p_method: 'bank' })
    expect(error).not.toBeNull()

    const { error: negError } = await fx.asAccountant.rpc('record_payment', { p_fee_id: fx.feeId, p_amount: -5, p_method: 'bank' })
    expect(negError).not.toBeNull()
  })

  it('يرفض دفعة تتجاوز المتبقّي على الفاتورة', async () => {
    const { error } = await fx.asAccountant.rpc('record_payment', { p_fee_id: fx.feeId, p_amount: 999, p_method: 'bank' })
    expect(error).not.toBeNull()
    expect(error?.message).toContain('يتجاوز')
  })

  it('يرفض فاتورة غير موجودة', async () => {
    const { error } = await fx.asAccountant.rpc('record_payment', {
  ...
  it('حارس التكرار: يرفض نفس المبلغ ونفس الطريقة لنفس الفاتورة خلال 10 ثوانٍ', async () => {
    const first = await fx.asAccountant.rpc('record_payment', { p_fee_id: fx.feeId, p_amount: 15, p_method: 'bank' })
    expect(first.error).toBeNull()

    const dup = await fx.asAccountant.rpc('record_payment', { p_fee_id: fx.feeId, p_amount: 15, p_method: 'bank' })
  ...
  it('يسمح بدفعتين مختلفتين (مبلغ مختلف) على نفس الفاتورة بدون تعارض حارس التكرار', async () => {
    ...
    const second = await fx.asAccountant.rpc('record_payment', { p_fee_id: fx.feeId, p_amount: 20, p_method: 'bank' })
...
describe('record_payment — الدفعات الجزئية المتراكمة', () => {
  it('ثلاث دفعات جزئية متتالية تصل بالضبط لإجمالي الفاتورة، لا أكثر ولا أقل', async () => {
    await fx.asAccountant.rpc('record_payment', { p_fee_id: fx.feeId, p_amount: 30, p_method: 'bank' })
    await new Promise((r) => setTimeout(r, 11000))
    await fx.asAccountant.rpc('record_payment', { p_fee_id: fx.feeId, p_amount: 30, p_method: 'cash' })
    await new Promise((r) => setTimeout(r, 11000))
    const last = await fx.asAccountant.rpc('record_payment', { p_fee_id: fx.feeId, p_amount: 40, p_method: 'bank' })
