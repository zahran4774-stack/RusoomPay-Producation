...
// - محدود بالمعدّل عبر دالة ذرّية check_and_increment_rate_limit (SECURITY DEFINER) —
//   [Phase 5A Fix] استبدلت SELECT/UPDATE المنفصلين (كانا يفشلان صامتاً بسبب صلاحيات
//   ناقصة على rate_limits، ما يعطّل الحدّ بالكامل) بعملية ذرّية واحدة تمنع أيضاً
//   تجاوز الحدّ عبر طلبات متزامنة (race condition).
//
...
## الأدوار والصلاحيات (تعرفها بدقة تامة)

في RusoomPay ستة أدوار، كل دور له صلاحيات محدّدة:

- **مدير المنصة (platform_admin)**: فوق كل المدارس. يرى ويدير كل المدارس المشتركة في RusoomPay، اشتراكاتها، وحالتها. لا يخصّ مدرسة واحدة.
- **مدير المدرسة (owner)**: الصلاحية الكاملة داخل مدرسته. وحده يصل لإعدادات نسب التأمينات، اعتماد طلبات تعديل الرواتب، الهيكل التنظيمي (تعديل)، وقرارات المدرسة الحسّاسة. يرى كل ما يراه الإداري والمحاسب، وأكثر.
- **إداري (admin)**: يدير العمليات اليومية — الطلاب، الرسوم، الموظفون، النقل، الشهادات، الإعلانات. لا يصل لبعض القرارات المحصورة بالمدير (مثل اعتماد الرواتب أو ضبط نسب التأمين).
- **محاسب (accountant)**: يصل للمحاسبة والتقارير المالية والرسوم، بجانب صلاحيات مشابهة للإداري في الجوانب المالية. التركيز على الجانب المالي تحديداً.
- **ولي الأمر (parent)**: يرى فقط بوابة أولياء الأمور (/parent) — أبناءه، أرصدتهم، ويسدّد إلكترونياً. لا يرى بيانات مدرسية إدارية أو طلاباً آخرين.
- **الطالب (student)**: دور محدود، يخصّ الطالب نفسه فقط.

**مجموعات صلاحيات شائعة يستخدمها النظام داخلياً:**
- **طاقم المدرسة (STAFF)**: مدير المدرسة + إداري + محاسب — هؤلاء الثلاثة يصلون لصفحات الإدارة اليومية.
- **الوصول المالي (FINANCE)**: مدير المدرسة + محاسب فقط — هؤلاء الاثنان يصلان للمحاسبة.
- **قرارات المدير فقط (OWNER_ONLY)**: مدير المدرسة وحده — لا الإداري ولا المحاسب.

عند سؤال المستخدم عن "من يستطيع كذا؟" أجب بدقة بناءً على هذا التصنيف — لا تخمّن.
...
// ---------- تحديد المعدّل — عملية ذرّية واحدة عبر دالة SECURITY DEFINER ----------
// [Phase 5A Fix] كان المنطق القديم SELECT ثم UPDATE منفصلين عبر عميل مربوط بجلسة
// المستخدم (RLS)؛ لأن دور authenticated لم يكن يملك GRANT أصلاً على rate_limits،
// كان كل استدعاء يفشل صامتاً (supabase-js لا يرمي استثناء، بل يُرجع {data:null,error})،
// والكود لم يكن يتحقق من error — فكانت الدالة تُرجع true دائماً: الحدّ كان معطَّلاً بالكامل.
// الحل: دالة SECURITY DEFINER ذرّية (INSERT ... ON CONFLICT DO UPDATE في عبارة واحدة)
// تتجاوز مشكلة الصلاحيات، وتغلق أيضاً فجوة تزامن الطلبات المتزامنة حول نفس الحدّ.
async function checkRateLimit(
  ...
  if (error) {
    // فشل تخزين المعدّل نفسه (مثلاً قاعدة البيانات غير متاحة مؤقتاً) — نغلق الوصول احتياطاً
    // (fail-closed) بدل السماح بلا حدود، حسب المطلوب صراحة عند فشل تخزين المعدّل.
    console.error('[assistant] rate_limit_check_failed:', error.message)
    return false
  }
  return data === true
}
