...
export default function LoginPage() {
  ...
  // جارٍ التحقّق من وجود جلسة سابقة (مثلاً بعد الضغط على رابط تأكيد البريد)
  ...
  async function finishLogin() {
    ...
    if (!myRole && user.user_metadata?.school_name) {
      ...
      const { error: rpcErr } = await supabase.rpc('register_school', {
        ...
        p_vat: m.vat || '', p_phone: m.phone || '', p_email: user.email || email, p_address: m.address || '',
  ...
  // عند فتح الصفحة: تحقّق هل فيه جلسة موجودة أصلاً (مثلاً المستخدم رجع لتوّه
  // من رابط تأكيد البريد الإلكتروني — Supabase تُنشئ الجلسة تلقائياً بالخلفية).
  // بدون هذا الفحص، الصفحة تعرض نموذج الدخول العادي وكأن شيئاً لم يحدث،
  // فيضطر المستخدم يدخل بريده وكلمة المرور يدوياً رغم أنه مسجَّل دخول فعلاً.
  useEffect(() => {
    let cancelled = false
  ...
  // أثناء الفحص التلقائي للجلسة (مثلاً بعد الرجوع من رابط تأكيد البريد)
  // نعرض شاشة انتظار واضحة بدل نموذج الدخول الفارغ — حتى لا يظن المستخدم
  // أن الصفحة توقّفت عن العمل.
  if (checkingSession) {
    return (
      <div style={{ minHeight: '100dvh', display: 'grid', placeItems: 'center', background: '#F4F6FA' }} dir="rtl">
        <div style={{ textAlign: 'center', color: '#556' }}>
          <div style={{
            width: 36, height: 36, margin: '0 auto 16px', borderRadius: '50%',
            border: '3px solid #DDE3EC', borderTopColor: '#163B68',
            animation: 'lp-spin 0.8s linear infinite',
          }} />
          <p style={{ fontSize: 14, fontWeight: 600 }}>جارٍ التحقّق من حسابك…</p>
          <style jsx>{`@keyframes lp-spin { to { transform: rotate(360deg) } }`}</style>
        </div>
      </div>
    )
  }
