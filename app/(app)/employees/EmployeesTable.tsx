...
function EditModal({ emp, role, onSave, onClose }: { emp: Emp; role: string; onSave: (e: Emp) => Promise<void>; onClose: () => void }) {
  ...
  // يقفل تمرير الصفحة خلف النافذة أثناء فتحها — يمنع مشكلة "التمرير يحرّك الصفحة بدل النافذة"
  // الشائعة على iOS Safari مع العناصر المنبثقة ذات fixed position
  useEffect(() => {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,39,68,.5)', display: 'grid', placeItems: 'center', padding: 16, zIndex: 100 }} dir="rtl" onClick={() => !saving && onClose()}>
      <div
        ...
        style={{
          background: '#fff', borderRadius: 16, width: 'min(92vw, 460px)', maxHeight: '88vh',
          display: 'flex', flexDirection: 'column', overflow: 'hidden',
        }}
      >
        {/* رأس ثابت */}
        <div style={{ padding: '20px 24px 12px', flexShrink: 0 }}>
          <h3 style={{ color: '#0F2744', margin: 0 }}>تعديل: {emp.full_name}</h3>
        </div>

        {/* المحتوى القابل للتمرير */}
        <div style={{ padding: '0 24px', overflowY: 'auto', WebkitOverflowScrolling: 'touch', flex: 1 }}>
          {role === 'accountant' && (
