  ...
  return (
    <div style={{ maxWidth: 1080, margin: '0 auto' }} dir="rtl">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(170px,1fr))', gap: 14, marginBottom: 14 }}>
        <Kpi label="الطلاب النشطون" value={int(data.students)} accent="#2E5EA8" icon={GraduationCap} />
        <Kpi label="الموظفون" value={int(data.employees)} accent="#0E5C5C" icon={Users} />
        {canFinance && <Kpi label="صافي الربح" value={fmt(data.profit)} unit={sym} accent={(data.profit ?? 0) >= 0 ? '#1A7A45' : '#C0392B'} icon={TrendingUp} />}
        {canFinance && <Kpi label="المتبقّي للتحصيل" value={fmt(data.outstanding)} unit={sym} accent="#D4A017" icon={Wallet} />}
      </div>
...
function Kpi({ label, value, unit, accent, icon: Icon }: { label: string; value: string; unit?: string; accent: string; icon: LucideIcon }) {
  return (
    <div className="ep-kpi" style={{ background: '#fff', borderRadius: 16, padding: 18, boxShadow: '0 1px 4px rgba(0,0,0,.07)', borderTop: `3px solid ${accent}` }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span style={{ color: '#8A94A6', fontSize: 13 }}>{label}</span>
        <Icon size={20} strokeWidth={2} color={accent} />
      </div>
      <div style={{ fontFamily: 'Cairo, sans-serif', fontWeight: 700, fontSize: 23, color: '#0F2744', marginTop: 8 }}>
