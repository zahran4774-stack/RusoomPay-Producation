...
// عرض الإيصال: receipt_url مسار داخلي في bucket خاص (subscription-receipts) — لا يُفتح مباشرة،
// بل عبر رابط موقّع مؤقت يُولَّد من /api/subscription-receipt-url عند الضغط.
...
export default function PendingSubs({ items }: { items: Item[] }) {
  ...
  // يولّد رابطاً موقّعاً مؤقتاً (5 دقائق) للإيصال الخاص، ثم يفتحه في تبويب جديد
  async function viewReceipt(id: string, path: string) {
    setLoadingReceipt(id)
    try {
      const res = await fetch('/api/subscription-receipt-url', {
        method: 'POST',
      ...
      if (!data.success || !data.url) {
        alert('تعذّر فتح الإيصال: ' + (data.error || 'خطأ غير معروف'))
        return
      }
      window.open(data.url, '_blank', 'noopener,noreferrer')
    } catch {
      alert('تعذّر الاتصال بالخادم لفتح الإيصال')
    } finally {
        ...
        <div key={it.id} style={{ background: '#FFFDF5', border: '1.5px solid #D4A017', borderRadius: 14, padding: 16 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
            <div>
              ...
              {it.receipt_url ? (
                <button
                  onClick={() => viewReceipt(it.id, it.receipt_url as string)}
                  disabled={loadingReceipt === it.id}
                  style={{
                    fontSize: 13, color: '#163B68', fontWeight: 600, marginTop: 6,
                    background: 'none', border: 'none', padding: 0, cursor: loadingReceipt === it.id ? 'default' : 'pointer',
                    textDecoration: 'underline', fontFamily: 'inherit',
                  }}>
                  {loadingReceipt === it.id ? '⏳ جارٍ الفتح…' : '📎 عرض الإيصال'}
                </button>
              ) : (
